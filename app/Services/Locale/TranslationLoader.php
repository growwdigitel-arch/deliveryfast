    ...
    public function load($locale, $group, $namespace = null)
    {
        ...
        if ($group === '*' && $namespace === '*') {
            return Cache::remember("locale.translations.{$locale}.{$group}", 60, function () use ($locale) {
                ...
                $database_translations = false;
                try {
                    if (Schema::hasTable('translations')) {
                        $database_translations = Translation::all();
                    }
                } catch (\Exception $e) {
                    // If there's an exception (e.g., DB connection down),
                    // we'll fall back to original_translations.
                    // $database_translations remains false.
                }
