...
class ShipmentController extends Controller
{
    ...
    private function storeShipment($request , $token = null)
    {
        ...
        if(isset($request->Shipment['shipping_cost']) && $request->Shipment['shipping_cost'] != null && $request->Shipment['shipping_cost'] > 0){
            $costs['shipping_cost'] = $request->Shipment['shipping_cost'];
        }
        if(isset($request->Shipment['tax']) && $request->Shipment['tax'] != null && $request->Shipment['tax'] > 0){
            $costs['tax'] = $request->Shipment['tax'];
        }
        if(isset($request->Shipment['insurance']) && $request->Shipment['insurance'] != null && $request->Shipment['insurance'] > 0){
            $costs['insurance'] = $request->Shipment['insurance'];
        }
        $model->fill($costs);
        if (!$model->save()) {
            return response()->json(['message' => new \Exception()] );
    ...
    public function tracking(Request $request)
    {
        if(empty($request->code)){
            return redirect()->back()->with(['error' => __('cargo::view.enter_your_tracking_code')]);
        }
        $shipment = Shipment::where('code', $request->code)->orWhere('order_id', $request->code)->first();
        
        if(empty($shipment)){
            return redirect()->back()->with(['error' => __('cargo::view.error_in_shipment_number')]);
        }
        $client = Client::where('id', $shipment->client_id)->first();    
        $PackageShipment = PackageShipment::where('shipment_id',$shipment->id)->get();
        $ClientAddress = ClientAddress::where('client_id',$shipment->client_id)->first();
        ...
        if($shipment){
            ...
            return redirect()->back()->with(['error' => __('cargo::messages.invalid_code')]);
        }
    }
    ...
    public function ajaxValidateTracking(Request $request)
    {
        if(empty($request->code)){
            return response()->json(['valid' => false, 'error' => __('cargo::view.enter_your_tracking_code')]);
        }

        $shipment = Shipment::where('code', $request->code)->orWhere('order_id', $request->code)->first();
        
        if(empty($shipment)){
            return response()->json(['valid' => false, 'error' => __('cargo::view.error_in_shipment_number')]);
        }

        return response()->json(['valid' => true, 'code' => $request->code]);
    }
